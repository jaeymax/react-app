...
const WorkExperience:React.FC<ModalProps> = ({closeModal}) => {
  ...
  return (
     <Modal name="Add Work Experience" closeModal={closeModal}  >
        <div className="bg-blue-100 px-10 py-3 flex items-center justify-between" >
           ...
           <button className="bg-black text-white p-4 font-bold w-36" >Link NSS</button>
         </div>
         <div className="flex-1 p-10 flex flex-col gap-5">
           <div className="flex flex-col w-full" >
             <label className="text-gray-500" htmlFor="job-title">Job Title</label>
             <input onChange={(e)=>setJobTitle(e.target.value)} value={jobTitle} className="border outline-none p-5 rounded-sm h-16" type="text" name="job-title" id="job-title" />
           </div>
           <div className="flex flex-col w-full" >
             <label className="text-gray-500" htmlFor="company">Company</label>
             <input onChange={(e)=>setCompany(e.target.value)} value={company} className="border outline-none p-5 rounded-sm h-14" placeholder="Search company" type="text" name="company" id="company" />
           </div>
          
           <div className="flex gap-3" >
           <div className="flex flex-col flex-1 min-w-0" >
             <label className="text-gray-500" htmlFor="start-date">Start date</label>
           ...
           <div className="flex gap-3">
             <input onChange={(e)=>setCurrentlyWorking(e.target.checked)} checked = {currentlyWorking} type="checkbox" name="work-here" id="work-here" />
